...
class OGD(Trainer):
    ...
    def update_mem(self, task_set, task_count):
        ...
        if self.algo_name == "pca-ogd":
            randind = torch.randperm(len(task_set))[
                      :self.pca_sample]  ## for pca method we  samples pca_samples > num_sample_per_task before applying pca and keeping (num_sample_per_task) elements
        else:
        ...
        if self.algo_name == "pca-ogd":
            ...
            try:
                ...
                _, _, v1 = torch.svd_lowrank((new_basis_tensor.T + 1e-4 * new_basis_tensor.T.mean() * torch.rand(
                    new_basis_tensor.T.size(0), new_basis_tensor.T.size(1))).cpu(), q=num_sample_per_task, niter=2,
                                             M=None)
    ...
    def _get_new_ogd_basis(self, train_loader, optimizer):
        ...
        for _, (x, y, t) in enumerate(train_loader):
            ...
            if self.test_label:
                out = out[2 * t:2 * (t + 1)]  # I added this, it works only for split mnist with 5 tasks
    ...
    def optimizer_step(self, ind_task):
        ...
        if self.test_label:
            ...
            cur_param = parameters_to_vector(self.get_params_dict(last=True, task_key=ind_task))
            grad_vec = parameters_to_grad_vector(self.get_params_dict(last=True, task_key=ind_task))
            cur_param -= self.lr * grad_vec
            vector_to_parameters(cur_param, self.get_params_dict(last=True, task_key=ind_task))
